...
@php
    $baseClasses = 'w-full px-4 py-2 bg-transparent border rounded-md text-sm transition-colors focus:outline-none focus:ring-1 resize-y min-h-[100px]';
    $stateClasses = $error 
...
<textarea {{ $disabled ? 'disabled' : '' }} {{ $attributes->merge(['class' => "$baseClasses $stateClasses $disabledClasses"]) }}>{{ $slot }}</textarea>
